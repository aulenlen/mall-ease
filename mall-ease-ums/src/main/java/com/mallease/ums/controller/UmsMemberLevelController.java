...
public class UmsMemberLevelController {
     ...
     */
    @GetMapping("/list")
    public R<List<UmsMemberLevel>> list(@RequestParam(required = false) Integer defaultStatus) {
     ...
     */
    @GetMapping("/{id}")
    public R<UmsMemberLevel> getById(@PathVariable Long id) {
        log.info("根据ID查询会员等级, id: {}", id);
        try {
            UmsMemberLevel memberLevel = memberLevelService.getById(id);
            if (memberLevel == null) {
     ...
     */
    @PostMapping("/create")
    public R<Integer> create(@RequestBody UmsMemberLevel memberLevel) {
     ...
     */
    @PostMapping("/update")
    public R<Integer> update(@RequestBody UmsMemberLevel memberLevel) {
