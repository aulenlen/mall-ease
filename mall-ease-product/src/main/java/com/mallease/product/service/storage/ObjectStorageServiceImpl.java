...
import com.mallease.product.config.BucketPolicyConfig;
...
public class ObjectStorageServiceImpl implements ObjectStorageService {
    ...
    @Override
    public ObjectUploadRespVO uploadFile(MultipartFile file) {
        try {
            ensureBucketExists();
            ...
            if (StrUtil.isNotBlank(publicUrl)) {
                ...
                // 回退到 endpoint（本地 S3 兼容场景）
                url = endpoint + "/" + bucketName + "/" + objectName;
            }

            return ObjectUploadRespVO.builder()
                    .url(url)
                    .name(objectName)
                    .build();
        } catch (Exception e) {
    ...
    private void ensureBucketExists() {
        try {
            boolean found = storageClient.bucketExists(BucketExistsArgs.builder()
                    .bucket(bucketName)
                    .build());
            if (!found) {
                ...
                if (!isR2Storage()) {
                    ...
                    SetBucketPolicyArgs setBucketPolicyArgs = SetBucketPolicyArgs.builder()
                            ...
                            .build();
                    storageClient.setBucketPolicy(setBucketPolicyArgs);
