...
public class ParamGroupServiceImpl implements ParamGroupService {
    ...
    @Autowired
    private CategoryParamGroupDao categoryParamGroupDao;

    @Override
    @Transactional(rollbackFor = Exception.class)
    public Long create(ParamGroup entity, Long categoryId) {
        // 检查名称是否重复
        ParamGroup existing = paramGroupDao.selectByName(entity.getName());
        if (existing != null) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int update(ParamGroup entity) {
        if (entity.getId() == null) {
            throw new ApiException("参数组ID不能为空");
        }

        ParamGroup original = paramGroupDao.selectByPrimaryKey(entity.getId());
        if (original == null) {
        ...
        if (StringUtils.hasText(entity.getName()) && !entity.getName().equals(original.getName())) {
            ParamGroup existingByName = paramGroupDao.selectByName(entity.getName());
    ...
    public int delete(Long id) {
        ParamGroup paramGroup = paramGroupDao.selectByPrimaryKey(id);
        if (paramGroup == null) {
            return 0;
        }

        // 检查是否有关联的参数定义
        List<Param> params = paramDao.selectByGroupId(id);
    ...
    public int deleteBatch(List<Long> ids) {
        ...
        // 检查是否有关联的参数定义
        List<Param> params = paramDao.selectByGroupIds(ids);
        if (!CollectionUtils.isEmpty(params)) {
            Long groupId = params.get(0).getGroupId();
            ParamGroup group = paramGroupDao.selectByPrimaryKey(groupId);
    ...
    @Override
    public ParamGroup getById(Long id) {
        return paramGroupDao.selectByPrimaryKey(id);
    }

    @Override
    public List<ParamGroup> listAll() {
        return paramGroupDao.selectAll();
    }

    @Override
    public List<ParamGroup> listEntities(String keyword) {
        return paramGroupDao.selectByKeyword(keyword);
    }

    @Override
    public List<ParamGroup> list(String keyword) {
        return paramGroupDao.selectByKeyword(keyword);
    }

    @Override
    public List<ParamGroup> listByCategoryId(Long categoryId) {
        // 通过关联表查询参数组ID
        List<CategoryParamGroup> relations = categoryParamGroupDao.selectByCategoryId(categoryId);
        if (CollectionUtils.isEmpty(relations)) {
            return new ArrayList<>();
        }

        List<Long> paramGroupIds = relations.stream()
                .map(CategoryParamGroup::getParamGroupId)
                .collect(Collectors.toList());
        return paramGroupDao.selectByIds(paramGroupIds);
    }

    @Override
    public Map<Long, List<Param>> getParamsByGroupIds(List<Long> groupIds) {
        if (CollectionUtils.isEmpty(groupIds)) {
            return Map.of();
        }

        List<Param> allParams = paramDao.selectByGroupIds(groupIds);
        return allParams.stream()
                .collect(Collectors.groupingBy(Param::getGroupId));
    }
    ...
    public int bindToCategory(Long categoryId, List<Long> paramGroupIds) {
        ...
        // 过滤已存在的关联
        List<CategoryParamGroup> existingRelations = categoryParamGroupDao.selectByCategoryId(categoryId);
        List<Long> existingGroupIds = existingRelations.stream()
                .map(CategoryParamGroup::getParamGroupId)
                .collect(Collectors.toList());
        List<CategoryParamGroup> newRelations = paramGroupIds.stream()
                .filter(groupId -> !existingGroupIds.contains(groupId))
                .map(groupId -> {
                    CategoryParamGroup relation = new CategoryParamGroup();
    ...
    public Long cloneToCategory(SaveParamGroupCmd cmd) {
        ...
        // 1. 查询原参数组
        ParamGroup sourceGroup = paramGroupDao.selectByPrimaryKey(sourceGroupId);
        if (sourceGroup == null) {
        ...
        // 检查名称是否重复，如果重复则添加时间戳
        ParamGroup existingGroup = paramGroupDao.selectByName(newName);
        if (existingGroup != null) {
            newName = newName + "_" + System.currentTimeMillis();
        }

        ParamGroup newGroup = new ParamGroup();
        ...
        // 3. 复制参数定义
        List<Param> sourceParams = paramDao.selectByGroupId(sourceGroupId);
