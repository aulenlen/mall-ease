...
import com.mallease.content.converter.ContentSubjectConverter;
...
public class ContentSubjectController {

    @Autowired
    private ContentSubjectService subjectService;

    @Autowired
    private ContentSubjectConverter subjectConverter;
    ...
    @PostMapping("/create")
    public R<Integer> create(@Validated @RequestBody CreateContentSubjectCmd cmd) {
        ContentSubject subject = subjectConverter.createCmdToEntity(cmd);
    ...
    public R<Integer> update(@PathVariable Long id, @Validated @RequestBody UpdateContentSubjectCmd cmd) {
        cmd.setId(id);
        ContentSubject subject = subjectService.getById(id);
        if (subject == null) {
            return R.failed(ResultCode.FAILED);
        }
    ...
    @GetMapping("/{id}")
    public R<ContentSubjectDetailVO> getById(@Parameter(description = "专题ID") @PathVariable Long id) {
        ContentSubject subject = subjectService.getById(id);
        ContentSubjectDetailVO vo = subjectConverter.entityToDetailVo(subject);
        return R.success(vo);
    }
    ...
    @GetMapping("/list")
    public R<Page<ContentSubjectListVO>> list(
        ...
        // 使用PageUtils转换分页结果
        Page<ContentSubjectListVO> result = PageUtils.convertPage(list, subjectConverter::entityListToListVoList);
    ...
    @GetMapping("/listAll")
    public R<List<ContentSubjectVO>> listAll() {
        List<ContentSubject> list = subjectService.list();
        List<ContentSubjectVO> voList = subjectConverter.entityListToVoList(list);
    ...
    public R<Page<ContentSubjectListVO>> listByCategoryId(
            @Parameter(description = "分类ID") @PathVariable Long categoryId,
            @Parameter(description = "页码") @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
            @Parameter(description = "每页数量") @RequestParam(value = "pageSize", defaultValue = "10") Integer pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        List<ContentSubject> list = subjectService.listByCategoryId(categoryId);
        List<ContentSubjectListVO> voList = subjectConverter.entityListToListVoList(list);
    ...
    public R<Page<ContentSubjectListVO>> listRecommend(
            @Parameter(description = "页码") @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
            @Parameter(description = "每页数量") @RequestParam(value = "pageSize", defaultValue = "10") Integer pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        List<ContentSubject> list = subjectService.listRecommend();
        List<ContentSubjectListVO> voList = subjectConverter.entityListToListVoList(list);
        return R.success(Page.restPage(voList));
    }
    ...
    public R<Integer> batchAddSpuRelation(@RequestBody List<ContentSubjectSpuRelation> relationList) {
    ...
    public R<List<ContentSubjectSpuRelation>> getRelationsBySpuId(
            @Parameter(description = "商品ID") @PathVariable("spuId") Long spuId) {
        List<ContentSubjectSpuRelation> list = subjectService.getRelationsBySpuId(spuId);
