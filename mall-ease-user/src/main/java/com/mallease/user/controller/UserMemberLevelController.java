...
public class UserMemberLevelController {
     ...
     */
    @GetMapping("/list")
    public R<List<UserMemberLevel>> list(@RequestParam(required = false) Integer defaultStatus) {
        log.info("查询会员等级列表, defaultStatus: {}", defaultStatus);
        try {
            List<UserMemberLevel> list;
     ...
     */
    @GetMapping("/{id}")
    public R<UserMemberLevel> getById(@PathVariable Long id) {
        log.info("根据ID查询会员等级, id: {}", id);
        try {
            UserMemberLevel memberLevel = memberLevelService.getById(id);
            if (memberLevel == null) {
     ...
     */
    @PostMapping("/create")
    public R<Integer> create(@RequestBody UserMemberLevel memberLevel) {
     ...
     */
    @PostMapping("/update")
    public R<Integer> update(@RequestBody UserMemberLevel memberLevel) {
