...
public class AddressPortalController {
    ...
    @GetMapping("/detail")
    public R<AddressRespVO> detail(@Parameter(description = "地址ID") @RequestParam Long id) {
        Long memberId = StpMemberUtil.getLoginIdAsLong();
    ...
    @PutMapping("/update")
    public R<Void> update(@Validated(AddressReqVO.Update.class) @RequestBody AddressReqVO reqVO) {
        Long memberId = StpMemberUtil.getLoginIdAsLong();
